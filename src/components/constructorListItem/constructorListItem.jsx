...
const ConstructorListItem = ({ constructor }) => {
    ...
    return (
        <Link to={`/constructors_list/${constructorId} `} className={"cursor-pointer border-t-2 border-r-2 border-stone-900 dark:border-gray-400 rounded-tr-xl dark:text-gray-300 pr-1 homePhone:pr-3 hover:border-red-600"}>
            ...
            <div className="flex justify-between border-b border-r rounded-br-xl border-gray-400 mb-4 pb-1">
                {
                    FindTeamDrivers(name).slice(0,2).map(driver => (
                        <div className="flex items-center justify-between text-base font-extrabold" key={driver.givenName}>
