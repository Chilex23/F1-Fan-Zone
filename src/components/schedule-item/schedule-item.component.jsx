...
const ScheduleItem = ({ details }) => {
    ...
    return (
        <>
            <motion.article className="cursor-pointer snap-center basis-[7.5rem] shrink-0 w-3/4 bg-stone-900 text-white border-b-2 border-r-2 border-stone-500 rounded-br-2xl py-20 h-[25rem] tablet:h-[30rem]" style={{flexBasis: animate ? "18rem" : "7.5rem"}} onClick={toggleAnimate}>
                <div>
                ...
                <motion.h1 className="text-lg tablet:text-xl font-extrabold text-center uppercase px-2" initial={{display: "none"}} animate={{display: animate ? "block" : "none", opacity: animate ? 1 : 0}} transition={timeTaken}>{name}</motion.h1>
