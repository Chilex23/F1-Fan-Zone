...
const TeamsMenu = () => {
    const { TEAMS } = useContext(NavBarContext);
    return (
        <div className="w-screen h-screen overflow-y-scroll bg-stone-800 fixed py-10 border-b-2 border-red-600 top-7 z-[8]">
            ...
            <div className="w-5/6 mx-auto grid grid-cols-3 grid-rows-4 mt-7 gap-7">
                {
                    TEAMS.map((elem, i) => <TeamLink key={i + 1} team={elem}/>)
                }   
            </div>
        </div>
    )
